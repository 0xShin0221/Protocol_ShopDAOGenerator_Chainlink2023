...
contract SortedProposalLists is AccessControl {
  /// Constants
  ...
  mapping(uint256 => uint256) _secondEarliestProposalId;
  ...
  function addProposalId(proposalInfo calldata params, uint256 _epochTime) public onlyRole(BRAND_MANAGER_ROLE)  {
  ...
  function removeProposalId(uint256 _proposalId) public onlyRole(BRAND_MANAGER_ROLE) onlyRegisteredAddress(_proposalId) {
    uint256 preOrderVault = _getPrevOrderProposalHash(_proposalId);
  ...
  function getEarlistEndOfDate() external view returns(uint, uint256) {
     return (proposalIds[_secondEarliestProposalId[GUARD]],_secondEarliestProposalId[GUARD]);
  }
  ...
  function _isPrevOrderProposalId(uint256 _proposalId, uint256 _preOrderProposalId) internal view returns(bool) {
    return _secondEarliestProposalId[_preOrderProposalId] == _proposalId;
  }

  function _getPrevOrderProposalHash(uint256 _proposalId) internal view returns(uint256) {
