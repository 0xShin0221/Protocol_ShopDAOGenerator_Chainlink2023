...
task(
  ...
  .setAction(async (taskArgs) => {
    ...
    const isWalletAllowed = await oracle.isAuthorizedSender(
      (
    ...
    const createWaitBlockConfirmations =
      consumer || linkAmount ? 1 : networks[network.name].confirmations;
    console.log(
    ...
    if (linkAmount) {
      ...
      const fundWaitBlockConfirmations = !!consumer
        ? 1
    ...
    if (consumer) {
      ...
      console.log(
        `Waiting ${
          networks[network.name].confirmations
        } blocks for transaction ${addTx.hash} to be confirmed...`
      );
