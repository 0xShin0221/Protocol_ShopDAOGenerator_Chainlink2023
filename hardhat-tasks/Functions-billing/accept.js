...
task(
  ...
  "Accepts ownership of an Functions subscription after a transfer is requested"
)
  ...
  .setAction(async (taskArgs) => {
    ...
    const RegistryFactory = await ethers.getContractFactory(
      "src/chainlink/functions/dev/devdevdevdevdevdevdevFunctionsBillingRegistry.sol:FunctionsBillingRegistry"
    );
    ...
    try {
      ...
      const acceptTx = await registry.acceptSubscriptionOwnerTransfer(
        subscriptionId
      );
      console.log(
        `Waiting ${
          networks[network.name].confirmations
        } blocks for transaction ${acceptTx.hash} to be confirmed...`
      );
    ...
    console.log(
      `Ownership of subscription ${subscriptionId} transferred to ${signerAddr}`
    );
