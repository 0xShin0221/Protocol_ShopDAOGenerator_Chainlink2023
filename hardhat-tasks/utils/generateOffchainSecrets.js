...
} = require("../../chainlink/FunctionsSandboxLibrary/encryptSecrets");

const generateOffchainSecrets = async (
  requestConfig,
  privateKey,
  DONPublicKey,
  nodeAddresses,
...
const validateRequestConfig = (requestConfig) => {
  ...
  if (
    requestConfig.perNodeSecrets &&
    !Array.isArray(requestConfig.perNodeSecrets)
  ) {
    throw Error(
      "perNodeSecrets is not correctly specified in config file.  It must be an array of objects."
    );
  }

  if (requestConfig.secrets && typeof requestConfig.secrets !== "object") {
    throw Error(
      "secrets object is not correctly specified in config file.  It must be an object."
    );
  }
  ...
  const globalSecretsObjectKeys = requestConfig.secrets
    ? Object.keys(requestConfig.secrets).sort()
    : undefined;
  ...
  if (requestConfig.perNodeSecrets && requestConfig.perNodeSecrets.length > 0) {
    const firstperNodeSecretsKeys = Object.keys(
      requestConfig.perNodeSecrets[0]
    ).sort();
    ...
    for (const assignedSecrets of requestConfig.perNodeSecrets) {
      ...
      if (Object.keys(assignedSecrets).length === 0) {
        throw Error(
          "In the config file, perNodeSecrets contains an empty object."
        );
      }
      ...
      if (
        ...
        JSON.stringify(globalSecretsObjectKeys) !==
          JSON.stringify(assignedSecretsObjectKeys)
      ) {
        throw Error(
      ...
      if (
        JSON.stringify(firstperNodeSecretsKeys) !==
        JSON.stringify(assignedSecretsObjectKeys)
      ) {
        throw Error(
