...
task(
  ...
  "Creates a JSON file with Functions request parameters"
)
  .addOptionalParam(
    "output",
    "Output file name (defaults to Functions-request.json)"
  )
  .addOptionalParam(
    "simulate",
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    fs.writeFileSync(
      taskArgs.output ?? "Functions-request.json",
      JSON.stringify(request)
    );
    console.log(
      `Wrote request to ${taskArgs.output ?? "Functions-request.json"}`
    );
  });
  ...
  for (const { secrets, url } of offchainSecretsResponses) {
    ...
    for (const nodeAddress of nodeAddresses) {
      if (!secrets[nodeAddress.toLowerCase()]) {
        if (!secrets["0x0"]) {
          throw Error(
            `No secrets specified for node ${nodeAddress.toLowerCase()} and no default secrets found.`
          );
        }
