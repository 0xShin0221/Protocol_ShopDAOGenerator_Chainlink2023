...
const { getRequestConfig } = require("../../chainlink/FunctionsSandboxLibrary");
...
const setAutoRequest = async (contract, taskArgs) => {
  ...
  if (doGistCleanup && request.secrets) {
    console.log(
      `Be sure to delete the Gist ${request.secretsURLs[0].slice(
        0,
        -4
      )} once encrypted secrets are no longer in use!\n`
    );
  }
  ...
  try {
    ...
    if (artifact.activeManagedSecretsURLs)
      previousSecretURLs = artifact.secretsURLs;
  } catch {
  ...
  console.log(
    `\nWaiting ${networks[network.name].confirmations} block for transaction ${
      setRequestTx.hash
    } to be confirmed...`
  );
