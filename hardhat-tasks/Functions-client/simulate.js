...
const { networks, SHARED_DON_PUBLIC_KEY } = require("../../chainlink/networks");
      ...
      registry.on(
        ...
        async (
          ...
          if (requestId == eventRequestId) {
            ...
            console.log(
              `Gas used by client callback function: ${fulfillGasUsed}`
            );
            return resolve();
          }
        }
...
const getGasUsedForFulfillRequest = async (success, result) => {
  ...
  let txReceipt;
  if (success) {
    txReceipt = await client.handleOracleFulfillment(
      simulatedRequestId,
      result,
      []
    );
