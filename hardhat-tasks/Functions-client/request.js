const {
  getDecodedResultLog,
  getRequestConfig,
} = require("../../chainlink/FunctionsSandboxLibrary");
const { generateRequest } = require("./buildRequestJSON");
const { networks } = require("../../chainlink/networks");
...
task(
  "functions-request",
  "Initiates a request from a Functions client contract"
)
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const unvalidatedRequestConfig = require(path.isAbsolute(
      taskArgs.configpath
    )
      ? taskArgs.configpath
    ...
    let doGistCleanup = !(
      requestConfig.secretsURLs && requestConfig.secretsURLs.length > 0
    );
      ...
      const cleanup = async () => {
        ...
        if (doGistCleanup) {
          if (!cleanupInProgress) {
            ...
            const success = await deleteGist(
              process.env["GITHUB_API_TOKEN"],
              request.secretsURLs[0].slice(0, -4)
            );
            if (success) {
              await store.update(requestId, {
                activeManagedSecretsURLs: false,
              });
            }
            return resolve();
          }
      ...
      registry.on(
        ...
        async (
          ...
          if (requestId == eventRequestId) {
            ...
            spinner.stop();
            console.log(
              `Actual amount billed to subscription #${subscriptionId}:`
            );
            const costBreakdownData = [
              {
                Type: "Transmission cost:",
                Amount: `${hre.ethers.utils.formatUnits(
                  eventTransmitterPayment,
                  18
                )} LINK`,
              },
              {
                Type: "Base fee:",
                Amount: `${hre.ethers.utils.formatUnits(baseFee, 18)} LINK`,
              },
              { Type: "", Amount: "" },
              {
                Type: "Total cost:",
